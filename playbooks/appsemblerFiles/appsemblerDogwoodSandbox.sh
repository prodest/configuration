...
##
sudo apt-get install -y build-essential software-properties-common python-software-properties curl git-core libxml2-dev libxslt1-dev python-pip python-apt python-dev libxmlsec1-dev libfreetype6-dev swig libmysqlclient-dev libffi-dev
...
#force version to Dogwood
export OPENEDX_RELEASE="named-release/dogwood"

## Did we specify an openedx release?
if [ -n "$OPENEDX_RELEASE" ]; then
  EXTRA_VARS="-e edx_platform_version="appsembler/feature/mergeDogwood.3" \
    -e edx_platform_repo="https://github.com/appsembler/edx-platform.git" \
    -e configuration_version="appsembler/feature/mergeDogwood.3" \
...
cd configuration
git checkout appsembler/feature/mergeDogwood.3
